...
void LikoMcrtcInterfPlugin::init(mc_control::MCGlobalController & controller, const mc_rtc::Configuration & config)
{
  ...
  // nh_ = &this_nh;
  bitbot_orientation_torso_ = Eigen::Quaterniond(0, 0, 0, 1);
  bitbot_position_torso_ = Eigen::Vector3d(0, 0, 0);
  ...
  if(disp_liko_coordinate_)
  {
    controller.controller().gui()->addElement(
        ...
        mc_rtc::gui::Transform("Marker",
                               [this]() {
                                 return sva::PTransformd{bitbot_orientation_torso_.conjugate(), bitbot_position_torso_};
                               }));
  }
...
void LikoMcrtcInterfPlugin::liko_callback(const nav_msgs::Odometry::ConstPtr & msg)
{
  ...
  bitbot_velocity_torso_ =
      Eigen::Vector3d(msg->twist.twist.linear.x, msg->twist.twist.linear.y, msg->twist.twist.linear.z);
  bitbot_orientation_torso_ = Eigen::Quaterniond(msg->pose.pose.orientation.w, msg->pose.pose.orientation.x,
